...
<%
    ...
    if (request.getParameter("rating_data") != null) {
        ...
        String selQry = "select count(*) as cid from tbl_review where provider_id='" + request.getParameter("labour_id") + "' and user_id='" + session.getAttribute("Urid") + "'";
        ResultSet rsd = con.selectCommand(selQry);
        rsd.next();
       
        if(rsd.getInt("cid")>0)
        {
            out.println("Your Review & Rating Already Submitted");
        }
        else
        {
             
        String ins = "INSERT INTO tbl_review(user_name,user_rating,user_review,review_datetime,provider_id,user_id)"
                + "VALUES('" + request.getParameter("user_name") + "','" + request.getParameter("rating_data") + "','" + request.getParameter("user_review") + "',SYSDATE(),'" + request.getParameter("labour_id") + "','"+session.getAttribute("Urid")+"')";
    ...
    if (request.getParameter("action") != null) {
        ...
        String query = "SELECT * FROM tbl_review where provider_id = '" + request.getParameter("lid") + "' ORDER BY review_id DESC";
